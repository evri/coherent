...
(function(){
  ...
  {
    return coordinate && new google.maps.LatLng(coordinate.latitude, coordinate.longitude);
  }
  ...
  {
    return bounds && new google.maps.LatLngBounds(toLatLng(bounds.northeast), toLatLng(bounds.southwest));
  }


  /**
    coherent.MapType -> Enumeration
  
    The coherent.MapType enumeration lists the valid types of map supported by
    Google Maps. The default value for new instances of coherent.MapView is
    Terrain.
   */
  coherent.MapType = {
    Hybrid: "hybrid",
  ...
  /**
    class coherent.MapView
  
    A view that wraps a Map from Google Maps.
   */
  coherent.MapView = Class.create(coherent.View, {

    exposedBindings: ['annotations', 'mapType', 'zoom', 'centerCoordinate', 'visibleMapRect'],
    ...
    constructor: function(node, parameters)
    {
      ...
      this.__mapType = coherent.MapType.Terrain;
      this.__zoom = 2;
      this.__minZoom = null;
      this.__maxZoom = null;
      this.__annotations = [];
    ...
    addAnnotations: function(annotations)
    {
      ...
      if (this.__hoveringInAnnotation || this.__clickedOnAnnotation || !this.__annotations.length)
        return;
    ...
    setMinimumZoom: function(minZoom)
    {
      ...
      if (!isNaN(this.__zoom) && !isNaN(this.__minZoom) && this.__zoom < this.__minZoom)
      {
    ...
    setMaximumZoom: function(maxZoom)
    {
      ...
      if (!isNaN(this.__zoom) && !isNaN(this.__maxZoom) && this.__zoom > this.__maxZoom)
      {
    ...
    setCenterCoordinate: function(position)
    {
      ...
      if (this.__map)
      {
        var latlng = new google.maps.LatLng(this.__centerCoordinate.latitude, this.__centerCoordinate.longitude);
    ...
    setVisibleMapRectForAddress: function(address)
    {
      ...
      this.__geocodeRequestComplete.bind(this));
    }
